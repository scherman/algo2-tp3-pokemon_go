...
public :
    ...
    void agregarCoor(tp3::Coordenada coor);

    const Conj<tp3::Coordenada>& coordenadas() const;

     bool posExistente(tp3::Coordenada coor) const;

     bool hayCamino(tp3::Coordenada coor1, tp3::Coordenada coor2) const;

private:
        Conj<tp3::Coordenada> validas;

static bool existeCamino(tp3::Coordenada coor1,tp3::Coordenada coor2,  Conj<tp3::Coordenada> & c){
    ...
    if( coor1 == coor2){ //caso base
      ...
      if (c.EsVacio())  //caso base
        ...
        tp3::Coordenada arriba = coor1;
        tp3::Coordenada abajo = coor1;
        tp3::Coordenada izquierda = coor1;
        tp3::Coordenada derecha = coor1;
...
void Mapa::agregarCoor(tp3::Coordenada coor){
validas.AgregarRapido(coor);
}

const Conj<tp3::Coordenada>& Mapa::coordenadas() const{
return validas;
}

bool Mapa::posExistente(tp3::Coordenada coor) const{
return validas.Pertenece(coor);
}

bool Mapa::hayCamino(tp3::Coordenada coor1, tp3::Coordenada coor2) const{
Conj<tp3::Coordenada> nuevo(validas);
